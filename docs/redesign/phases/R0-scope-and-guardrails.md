...
- [ ] Read `00-START-HERE.md`, `SOLAR-SYSTEM-SPEC.md`, `BIOME-AND-LOADING-SPEC.md`,
...
- [ ] Also read, from the existing repo: root `CLAUDE.md`, `SKILL-TREE-3D.md`,
      `GAME-DESIGN.md` §1-9, `DESIGN-MANDATE.md` (the original), `PAGE-SPECS.md`
- [ ] **Also read the real data these docs make claims about, not just the
      ...
      forever
- [ ] Restate the scope boundary in your own words: what's redone, what's
...
- [ ] Confirm no new client-side mastery/lock computation exists in
      `apps/web` today, so any INV-34 regression later is attributable to
      this track specifically, not inherited
- [ ] Locate the cosmetic-endpoint path per `SOLAR-SYSTEM-SPEC.md` §3
...
- [ ] Record the real curriculum numbers as durable facts in
...
- [ ] Check what actually exists before assuming: is `@playwright/mcp`
...
- [ ] If `@playwright/test` isn't present: install at the workspace root,
...
- [ ] Create `design/templates/`, `design/baselines/`, `design/screenshots/`,
      `design/qa-report/`; gitignore the latter two
- [ ] **Guard rail beyond the folders themselves:** `design/baselines/` is
      ...
      permanent
- [ ] Run a smoke capture of the current (galaxy) `/app` and `/app/map` at
      ...
      later
- [ ] Note for later phases: if this repo's demo/seed data gets reset by a
...
- [ ] Move the redesign package's files into `docs/redesign/` per
      `FOLDER-STRUCTURE.md`, if they aren't there already — check first,
      don't assume either state
- [ ] Leave `ADAPTIVE-SCORING-PROPOSAL.md` **outside** `docs/redesign/`, per
      that file's own header and `FOLDER-STRUCTURE.md`
- [ ] Fix any cross-references that assumed a file exists that doesn't yet
...
- [ ] This move is the cheap moment to do it, before anything is tracked
      against the old paths — do it as this phase's first commit

## R0.5 — `docs/PROGRESS.md`
- [ ] Update it (it may already exist from an earlier partial run — check
...
- [ ] First commit for this track: docs + Playwright config + the
      folder move from R0.4 — no feature code in this commit

## Definition of done
- [ ] Scope boundary restated correctly and confirmed
- [ ] Every open decision from R0.1b has an explicit answer, recorded
- [ ] Playwright verified working (both the MCP and `@playwright/test`
      states known, not assumed), folder structure created, before-baselines
      captured for `/app` and `/app/map` at both widths
- [ ] The package lives at `docs/redesign/` (or its actual current location
      is confirmed correct and documented), cross-references fixed
- [ ] `docs/PROGRESS.md` exists and is current
