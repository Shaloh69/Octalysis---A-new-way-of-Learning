...
>
> **This file sat at 0 of 36 ticked long after the phase was complete.**
...
>
> **R1 is complete.** All 36 items ticked.
>
> Ticked means *evidence exists in this repository*, not *remembered as done*.
> The evidence for each group is named below.

> **Evidence.** `apps/web/src/solar-system/layout.ts` is the pure function, with
> F-1's named fallback, F-2's occupancy spacing, F-3's ~300° sweep and F-4's
> all-levels rendering each implemented and commented. `apps/web/test/` carries
> **53 unit tests** including the moon-aggregation, ring-monotonicity and
> seed-independence assertions. 3D is the default on capable devices and the
> degradation ladder is asserted in `design/specs/solar-system.spec.ts`.
>
> **Performance, measured rather than assumed:** 3D chunk **220.6 KB gzipped**
> against the ≤250 KB budget, **21 draw calls** against ≤50, and **60.1 / 57.7
> fps** at 1× / 6× CPU throttle.
>
> One wording note kept rather than silently fixed: the DoD line says
> "INV-32-equivalent holds". **`INV-32` does not exist** — the invariant set stops
> at INV-31. The layout unit tests are what actually hold that line, and
> `DELIVERY.md` §3.1 cites the same non-existent invariants. Recorded in
> `PROGRESS.md`.
---
...
- [x] `layout.ts` — pure function, `stages[] + objectives[] -> Map<stageId | objectiveId, Vec3>`.
...
- [x] **F-1 · Stage 00 fallback, stated not improvised.** Stage 00 has zero
...
- [x] **F-3 · angle sweeps ~300°, not 360°.** `ordinal` maps across roughly
...
- [x] **F-2 · ring radii are spaced by occupancy, not evenly.** Measured: L1
...
- [x] **F-4 · Stage 01 keeps its all-levels rendering.** It declares
...
- [x] **Make 3D the default on capable devices.** Remove the default-off
...
- [x] **`/app` and `/app/map` stay the same component**, `flat` prop and all.
...
- [x] DOM canonical layer: real buttons per planet, focusable, labelled with
      title/state/lock-reason, ring/planet language instead of star/node
      language in the copy
- [x] WebGL presentation layer: `aria-hidden`, `pointer-events: none` on the
      canvas, DOM overlay handles all interaction — same overlay technique as
      before, per `SKILL-TREE-3D.md` §4
- [x] Verify: disable WebGL in the browser, confirm `/app` still works. **This
...
- [x] Verify: `prefers-reduced-motion` freezes to static — every ring and planet
...
- [x] Turn R0's recorded matrix into hard assertions now that the rules are
...
- [x] **Docs housekeeping this ruling creates:** edit the four documents above
...
## R1.3 — Base rendering
- [x] Sun at the center (procedural material, not a texture — see
      `SOLAR-SYSTEM-SPEC.md` §4.1)
- [x] Seven ring outlines, unlabelled until Stage 11 per §1.2
- [x] 19 planets, positioned via `layout.ts`, illuminated/dim per
      `is_stage_unlocked()` and `stage_progress.mastery` — read-only, never
      computed client-side
- [x] Flight path: one traced line through curriculum order, per §1.3,
      draw-progress reflecting actual student advancement
- [x] Background starfield, instanced `<Points>`, per existing budget (≤3,000
      points, one draw call)
- [x] Camera: ambient drift. (No `OrbitControls` — drei is not installed, and
      `VISUAL-SYSTEM-3D.md` §5 records it was dropped deliberately.)
- [x] **Planet click → camera flies to it (`GAME-DESIGN.md` §3.2 easing) →
...
- [x] Bundle size of the whole 3D chunk, gzipped, checked against the ≤250 KB
      budget — before adding cosmetics or moons, so you know the headroom
      remaining for R2/R4
- [x] Draw call count checked against ≤50
- [x] `frameloop="demand"` confirmed — canvas stops rendering when idle or tab
      hidden
- [x] Frame rate on a throttled/mid-range profile (Chrome DevTools CPU
...
- [x] Both layers render correctly, WebGL-disabled and reduced-motion paths
      verified by actually toggling them
- [x] Layout unit tests passing, INV-32-equivalent holds
- [x] Performance budget checked and under budget with headroom noted for R2/R4
      — bundle size and draw calls
- [x] **Planet click → camera fly-in → the §2 HUD, verified and screenshotted.
...
- [x] **FRAME RATE measured on a throttled / mid-range profile, against the
...
- [x] Screenshot captured and compared against the "before" galaxy screenshot
